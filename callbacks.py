...
from user_preferences import (load_preferences, update_recent_files, update_favorite_signals, 
                            ...
                            save_file_path_set, get_saved_file_paths, delete_saved_file_path_set,
...
def register_callbacks(app):
    ...
    # Add callback to toggle the visibility of saved paths section
    @app.callback(
    ...
    # Register new callbacks for saved file paths
    ...
    def save_current_paths(n_clicks, file_paths_text, path_set_name):
        ...
        try:
            print(f"DEBUG: save_current_paths callback triggered! n_clicks={n_clicks}")
            print(f"DEBUG: path_set_name='{path_set_name}'")
            print(f"DEBUG: file_paths_text length={len(file_paths_text) if file_paths_text else 0}")
            ...
            if not file_paths_text:
                print("DEBUG: file_paths_text is empty")
                return no_update, no_update, html.Div("No file paths to save", style={"color": "red"})
                
            if not path_set_name:
                print("DEBUG: path_set_name is empty")
                return no_update, no_update, html.Div("Please enter a name for the path set", style={"color": "red"})
            ...
            if not file_paths:
                ...
                return no_update, no_update, html.Div("No valid file paths found", style={"color": "red"})
            
            # Save the file paths
            print(f"DEBUG: Calling save_file_path_set with name='{path_set_name}' and {len(file_paths)} paths")
            success = save_file_path_set(path_set_name, file_paths)
            print(f"DEBUG: save_file_path_set returned {success}")
            
            # Update the dropdown options
            saved_paths = get_saved_file_paths()
            print(f"DEBUG: After save, get_saved_file_paths returned {len(saved_paths)} entries: {list(saved_paths.keys())}")
            options = [{"label": name, "value": name} for name in saved_paths.keys()]
            
            if success:
                status = html.Div(f"Saved file paths as '{path_set_name}'", style={"color": "green"})
                print(f"DEBUG: Returning success message: {status}")
                return options, "", status
            else:
                status = html.Div(f"Failed to save path set '{path_set_name}'", style={"color": "red"})
            ...
            print(traceback.format_exc())
            return no_update, no_update, html.Div(f"Error: {str(e)}", style={"color": "red"})
    ...
    def toggle_rename_modal(open_clicks, cancel_clicks, confirm_clicks, is_open):
