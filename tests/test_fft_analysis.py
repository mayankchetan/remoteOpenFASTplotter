...
class TestFFTAnalysis:
    ...
    def test_perform_fft(self, sine_wave_df):
        ...
        assert any(np.isclose(p, 10.0, atol=2.0)
                   for p in peaks), f"No peak found near 10 Hz, peaks: {peaks}"
        assert any(np.isclose(p, 50.0, atol=2.0)
                   for p in peaks), f"No peak found near 50 Hz, peaks: {peaks}"
        assert any(np.isclose(p, 120.0, atol=2.0)
                   for p in peaks), f"No peak found near 120 Hz, peaks: {peaks}"
    ...
    def test_perform_welch(self, sine_wave_df):
        ...
        # Note: Welch method smooths the spectrum, so peaks might be less
        # pronounced
        peaks = []
        for i in range(1, len(result.freq) - 1):
            if (result.amplitude[i] > result.amplitude[i - 1] and
        ...
        assert any(np.isclose(p, 10.0, atol=5.0)
                   for p in peaks), f"No peak found near 10 Hz in Welch, peaks: {peaks}"
        assert any(np.isclose(p, 50.0, atol=5.0)
                   for p in peaks), f"No peak found near 50 Hz in Welch, peaks: {peaks}"
...
def test_perform_binning():
    ...
    binned_freq_z, binned_psd_z = perform_binning(
        freq_with_zero, psd_with_zero, bins_per_decade=10)
