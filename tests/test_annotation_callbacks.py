...
# Instead, we will test the core logic if we can isolate it or simulate the callback's behavior.
...
# Mocking user_preferences and utils for now, as they are side effects or UI formatting
@pytest.fixture(autouse=True)
def mock_dependencies(mocker):
    mocker.patch('callbacks.annotation_callbacks.save_custom_annotations', return_value=None)
    mocker.patch('callbacks.annotation_callbacks.create_annotation_badges', return_value=[]) # Returns empty list for badges
...
def test_manage_annotations_add_to_existing():
    ...
    # Simplified logic
    freqs = [float(f.strip()) for f in freq_input.split(",") if f.strip()]
    labels = [l.strip() for l in label_input.split(",")] # Assuming label provided
...
def test_manage_annotations_auto_labels():
    ...
    labels_parsed = [l.strip() for l in label_input_partial.split(",")]
    if len(labels_parsed) < len(freqs2):
        labels_parsed += [f"F{i+1}" for i in range(len(labels_parsed), len(freqs2))]
...
def test_remove_annotation_logic():
    ...
    new_annotations_list = [a for i, a in enumerate(current_annotations) if i != index_to_remove]
...
def test_generate_rotor_harmonics_logic():
    ...
    for h_num in defined_harmonics:
        ...
        if not exists:
            new_annotations_list.append({"freq": harmonic_freq, "label": harmonic_label})
...
def test_generate_rotor_harmonics_avoids_duplicates():
    ...
    for h_num in defined_harmonics: # [1, 2, 3]
        ...
        if not exists:
            new_annotations_refined.append({"freq": harmonic_freq, "label": harmonic_label})
