...
def save_file_path_set(name, file_paths):
    ...
    # Debug information
    print(f"DEBUG: save_file_path_set called with name='{name}', paths={file_paths}")
    print(f"Current saved paths before update: {preferences.get('saved_file_paths', {})}")
    ...
    if result:
        print(f"Successfully saved path set '{name}' with {len(file_paths)} paths")
        # Verify by reloading
        ...
        if name in reloaded.get("saved_file_paths", {}):
            ...
            print(f"FAILURE: path set '{name}' not found in reloaded preferences!")
    else:
