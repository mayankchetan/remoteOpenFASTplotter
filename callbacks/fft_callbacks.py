...
def register_fft_callbacks(app):
    ...
    def calculate_fft(
        ...
        if not file_paths or not DATAFRAMES:
            return html.Div(
                "No files loaded", className="text-center p-5 text-muted"), None

        if not time_col:
            return html.Div(
                "Please select a time column (X Signal)", style={
                    "color": "red"}), None

        if not signals or len(signals) == 0:
            return html.Div(
                "Please select at least one signal (Y Signal)", style={
                    "color": "red"}), None
        ...
        if plot_style == "overlay":
            ...
            for signal_idx, signal in enumerate(signals):
                for file_idx, file_path in enumerate(file_paths):
                    ...
                    try:
                        ...
                        print(
                            f"Error in FFT calculation for {file_path}, signal {signal}: {e}")
            ...
            if annotations:
                for anno in annotations:
                    ...
                    fig.add_annotation(
                        ...
                        # Include frequency with 2 decimal places
                        showarrow=False,
            ...
            layout = dbc.Row([dbc.Col(dcc.Graph(
                figure=fig, id="main-fft-graph", config={'displayModeBar': True}), width=12)])
        ...
        # SEPARATE PLOT STYLE - One plot per signal
            ...
            if not fft_panels:
                return html.Div(
                    "No valid FFT results could be calculated. Please check your signal selections.", className="alert alert-warning"), None
