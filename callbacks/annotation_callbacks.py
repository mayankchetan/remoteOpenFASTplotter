...
def register_annotation_callbacks(app):
    ...
    def manage_fft_annotations(
        ...
        if trigger == "tabs":
            ...
            return current_annotations, create_annotation_badges(
                current_annotations), None, None
        ...
        try:
            freqs = [float(f.strip())
                     for f in freq_input.split(",") if f.strip()]
        except ValueError:
        ...
        # Parse labels - if not provided or fewer than freqs, generate
        # automatic labels
        if not label_input:
    ...
    def remove_annotation(n_clicks, current_annotations):
        ...
        if triggered_id and "index" in triggered_id:
            ...
            new_annotations = [a for i, a in enumerate(
