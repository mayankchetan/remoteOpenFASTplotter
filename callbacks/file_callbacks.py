...
def register_file_callbacks(app):
    ...
    def load_files_from_input(
        ...
        if trigger_id == "clear-files-btn":
            ...
            return {}, [], html.Div("All files cleared"), "", html.Div(), "0", {
                "display": "none"}, {"display": "none"}, [], False, {}

        # Handle reload files button click
        if trigger_id == "reload-files-btn":
            if not current_file_paths:
                return current_loaded_files, current_file_paths, html.Div(
                    "No files to reload", style={
                        "color": "red"}), "", create_file_pills(current_file_paths), str(
                    len(current_file_paths)), {
                    "display": "none"}, {
                    "display": "none"}, [], False, {}
            ...
            if valid_paths:
                new_dfs, failed_files, load_times = store_dataframes(
                    valid_paths)

                # Log loading times
                for file_path, elapsed in load_times.items():
                    print(
                        f"Reloaded {os.path.basename(file_path)} in {elapsed:.2} seconds")
                ...
                if failed_paths:
                    status_elements.append(
                        html.Span(
                            f" | ⚠️ {len(failed_paths)} files failed to reload",
                            style={
                                "color": "red",
                                "marginLeft": "10px"}))
                ...
                if invalid_paths:
                    status_elements.append(
                        html.Span(
                            f" | ⚠️ {len(invalid_paths)} files not found",
                            style={
                                "color": "red",
                                "marginLeft": "10px"}))
                ...
                error_container_style = {
                    "display": "block"} if has_errors else {
                    "display": "none"}
                error_button_style = {
                    "display": "block"} if has_errors else {
                    "display": "none"}
                    ...
                    for file_path in reloaded_files:
                        if file_path in DATAFRAMES:
                            ...
                            if 'Time' in df.columns or 'Time_[s]' in df.columns:
                                ...
                                if 'min_time' not in time_range_info or min_time < time_range_info[
                                        'min_time']:
                                    time_range_info['min_time'] = min_time
                                if 'max_time' not in time_range_info or max_time > time_range_info[
                                        'max_time']:
        ...
        if not file_paths_input:
            ...
            return current_loaded_files, current_file_paths, html.Div(
                "No file paths entered", style={
                    "color": "red"}), "", create_file_pills(current_files), str(
                len(current_files)), {
                "display": "none"}, {
                    "display": "none"}, [], False, {}
        ...
        new_files_to_process = [
            f for f in new_file_paths if f not in current_files]

        if not new_files_to_process:
            return current_loaded_files, current_file_paths, html.Div(
                "All files already loaded", style={
                    "color": "blue"}), "", create_file_pills(current_files), str(
                len(current_files)), {
                "display": "none"}, {
                    "display": "none"}, [], False, {}
        ...
        if valid_paths:
            ...
            for file_path, elapsed in load_times.items():
                print(
                    f"Loaded {os.path.basename(file_path)} in {elapsed:.2} seconds")
            ...
            if new_dfs:
                status_elements.append(
                    html.Span(
                        f"✓ Loaded {len(new_dfs)} new files",
                        style={
                            "color": "green"}))
            ...
            if failed_paths:
                status_elements.append(
                    html.Span(
                        f" | ⚠️ {len(failed_paths)} files failed to parse",
                        style={
                            "color": "red",
                            "marginLeft": "10px"}))
        ...
        if updated_files:
            if updated_files[0] in DATAFRAMES:
                ...
                signal_options = sorted(
                    list(DATAFRAMES[updated_files[0]].columns))

        # Clear the plot if no files are left
        plot_output = html.Div(
            "Select signals to plot",
            className="text-center p-5 text-muted") if not updated_files else no_update

        return ({"files": updated_files}, updated_file_paths, file_pills,
                file_count, signal_options, signal_options, plot_output)
